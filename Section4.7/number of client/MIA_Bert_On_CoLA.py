...
batch_sizes = [16]
epoch = 20
client_numbers = [5, 10, 30, 50, 100, 200]
local_steps = 1
attack_num = 100
...
for client_number in client_numbers:
    ...
    for batch_size in batch_sizes:
        ...
        for current_attack_number in range(attack_num):
            ...
            local_model.load_state_dict(global_model.state_dict())             # 当前batch训练前的模型
            positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True, return_tensors='pt').to(device)
            negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True, return_tensors='pt').to(device)
            with torch.no_grad():
        ...
        # 保存为 JSON 文件
        file_path = f'results/mia_bert_cola_results_{batch_size}_epoch_{epoch}_client_{client_number}.json'

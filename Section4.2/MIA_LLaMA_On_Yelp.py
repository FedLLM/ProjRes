from auxiliary_function import llama3_with_adapter, utility_function
import torch
import random
...
# 设置模型
num_labels = 10
model_path = "E:/Model/Llama3-8B-Instruct"
temp = torch.tensor([1]).to(device)
local_model = AutoModelForCausalLM.from_pretrained(model_path)
local_model.model.layers = local_model.model.layers[0:2]
adapter_size = int(local_model.model.embed_tokens.embedding_dim / Adapter_reduction_factor)
llama3_with_adapter.insert_transformer_adapters(local_model, adapter_size=adapter_size)
llama3_with_adapter.insert_classification_head(local_model, num_classes=num_labels)
llama3_with_adapter.freeze_model_parameters(local_model)
...
data_name = 'E:/Dataset/YELP/yelp_academic_dataset_tip/yelp_academic_dataset_tip.json'
train_raw_data, test_raw_data = utility_function.preprocess_yelp(data_name)
dataset = utility_function.CustomDataset(train_raw_data)
...
for batch_size in batch_sizes:
    ...
    train_loader_list = [DataLoader(client_dataset, batch_size=batch_size, shuffle=True, generator=generator)
    ...
    for current_epoch in range(epoch):
        ...
        for current_client in range(client_number):
            ...
            llama3_with_adapter.set_trainable_parameters(local_model)
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            ...
            if current_client == 0:
                for layer in local_model.model.layers:
            ...
            adapter_grads = []
            for block in local_model.model.layers:
        ...
        # 注册钩子函数，用于捕获model前向传播时，adapter0的输入
        local_model.load_state_dict(global_model)
        layer_hook = []
        for layer in local_model.model.layers:
        ...
        del positive_input, negative_input, negative_captured, positive_captured, weight_divided_bias, \
            adapter_input_captured, client_weights
        ...
        for block in local_model.model.layers:
            positive_cosine_grade.append(
                [block.adapter.adapter_down.weight.grad, block.adapter.adapter_down.bias.grad,
                 block.adapter.adapter_up.weight.grad, block.adapter.adapter_up.bias.grad])
        ...
        negative_u_average = sum(negative_sample_cosine[1:]) / max(1, len(negative_sample_cosine[1:]))
    ...
    # 保存为 JSON 文件
    file_path = f'results/mia_llama_yelp_results_{batch_size}.json'

...
def convert_hf_dataset_to_custom(hf_dataset):
    # 直接提取字段
    data_dict = {
        'label': hf_dataset['label'],  # list of integers
        'sentence': hf_dataset['sentence']  # list of strings
    }
    return data_dict
...
def prune_smallest_percent1(attn_grade, per=0.01):
    ...
    k = int(len(all_abs_values) * per)  # 最小的 1%
    if k == 0:
        k = 1
    ...
    for t in attn_grade:
        ...
        pruned_t = t * mask  # 或者 t.data *= mask; 如果你想 in-place 修改

...
def insert_transformer_adapters(model, reduction_factor, layer_start=0, layer_end=12):
    embedding_dim = model.bert.config.hidden_size
    for i, layer in enumerate(model.bert.encoder.layer[layer_start:layer_end]):
...
def insert_classification_head(model, num_classes):
    ...
    def forward_with_classification_head(self, input_ids, *args, **kwargs):
        ...
        pooled_output = hidden_states[:, 0, :]  # 取 [CLS] 位置的隐藏状态

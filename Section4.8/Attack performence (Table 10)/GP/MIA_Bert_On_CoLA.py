...
for per in per_s:
    ...
    for batch_size in batch_sizes:
        ...
        for current_attack_number in range(attack_num):
            ...
            grade_pure = []
            for temp in grade:
                temp0 = utility_function.prune_smallest_percent([temp[0]], per=per)[0]
                temp1 = utility_function.prune_smallest_percent([temp[1]], per=per)[0]
                grade_pure.append([temp0, temp1])

            weight_divided_bias = []
            for grade_temp in grade_pure:
            ...
            P_MIA_positive_sample_res.append(min(temp_res).item())               # 抽取最小值留存
        ...
        # 保存为 JSON 文件
        file_path = f'results/mia_bert_cola_results_{batch_size}_per_{per}.json'

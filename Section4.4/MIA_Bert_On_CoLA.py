...
batch_sizes = [16]
epoch_set = [1, 10, 20, 40, 100, 200, 400]
client_number = 30
local_steps = 1
attack_num = 100
...
Bert_insert_Adapter.insert_transformer_adapters(global_model, reduction_factor=Adapter_reduction_factor, layer_start=0, layer_end=12)
...
Bert_insert_Adapter.insert_transformer_adapters(local_model, reduction_factor=Adapter_reduction_factor, layer_start=0, layer_end=12)
...
raw_data = utility_function.preprocess_tsv(data_name)  # 1. 加载并预处理数据
dataset = utility_function.CustomDataset(raw_data)     # 转为 PyTorch Dataset
total_size = len(dataset)                              # 2. 计算每个客户端的数据量
base_size = total_size // client_number
extra = total_size % client_number                     # 多余的样本分配给前几个客户端
data_name = 'E:/Dataset/cola_public_1.1/cola_public/raw/in_domain_train.tsv'
test_raw_data = utility_function.preprocess_tsv(data_name)       # 1. 加载并预处理数据
test_dataset = utility_function.CustomDataset(test_raw_data)     # 转为 PyTorch Dataset
...
for epoch in epoch_set:
    ...
    for current_attack_number in range(attack_num):
        ...
        for index, temp_captured in enumerate(adapter_inputs):
            is_close, _, _, res_atol_final = utility_function.can_be_expressed(grade[index][0], temp_captured[1][0][0].to(device))
        ...
        # ===============================Fed_loss(ICLR, 2023)=================================
        local_model.load_state_dict(fed_loss_local_model.state_dict())
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True, return_tensors='pt').to(device)
        with torch.no_grad():
        ...
        local_model.load_state_dict(fed_loss_local_model.state_dict())  # 当前batch训练后的模型
        with torch.no_grad():
            positive_output_new = local_model(**positive_input)
    ...
    # 保存为 JSON 文件
    file_path = f'results\mia_bert_cola_results_{batch_size}_epoch_{epoch}.json'

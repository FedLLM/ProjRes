...
for batch_size in batch_sizes:
    ...
    current1 = torch.cuda.memory_allocated(device)
    mem_use_local_model = current1 - current
    ...
    for current_epoch in range(epoch):
        ...
        for current_client in range(client_number):
            ...
            Qwen_insert_LoRA.set_trainable_parameters_DownProj(local_model)
            ...
            trainable_params_cpu = [p.detach().cpu() for p in local_model.parameters() if p.requires_grad]
            if current_client == 0:
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            ...
            if current_client == 0:
                for layer in local_model.layers:
                    ...
                    weight_grad = down_proj.weight.grad
                    weight_divided_bias.append(weight_grad)

            adapter_grads = []
            for block in local_model.layers:
                adapter_grads.append([block.mlp.down_proj.weight.grad])
        ...
        layer_hook = []
        for layer in local_model.layers:
            temp_hook = layer.mlp.down_proj.register_forward_pre_hook(capture_adapter_input)
        ...
        positive_temp_res = []
        negative_temp_res = []
        for index, temp_captured in enumerate(positive_captured):
            _, _, _, res_atol_final = utility_function.can_be_expressed(weight_divided_bias[index], temp_captured[0][-1].to(device))
            positive_temp_res.append(res_atol_final.item())
            _, _, _, res_atol_final = utility_function.can_be_expressed(weight_divided_bias[index], negative_captured[index][0][-1].to(device))
        ...
        for block in local_model.layers:
            positive_cosine_grade.append(
                [block.mlp.down_proj.weight.grad])
        ...
        local_model.load_state_dict(global_model)  # 当前batch训练前的模型
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        ...
        positive_grads = []
        for block in local_model.layers:
            positive_grads.append([block.mlp.down_proj.weight.grad])
        ...
        negative_grads = []
        for block in local_model.layers:
            negative_grads.append([block.mlp.down_proj.weight.grad])
        ...
        positive_sample_cosine = []  # step 1 开始
        ...
        positive_u_average = sum(positive_sample_cosine[1:]) / max(1, len(positive_sample_cosine[1:]))  # step 2 开始
        ...
        positive_z_score = (positive_sample_cosine[0] - positive_U_out) / np.sqrt(positive_V_out)  # step 3 开始
    ...
    # 保存为 JSON 文件
    file_path = f'results/mia_Qwen_DownProj_results_{batch_size}.json'
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=4, ensure_ascii=False)

    print(f"✅ 当前batch size结果已保存至 '{file_path}'")

    final_result.append([P_MIA_positive_sample_res, P_MIA_negative_sample_res, P_MIA_Memory_use, P_MIA_Time_use])

file_path = f'results/mia_Qwen_DownProj2_results.json'
with open(file_path, 'w', encoding='utf-8') as f:

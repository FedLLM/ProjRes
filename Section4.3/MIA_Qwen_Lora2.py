from auxiliary_function import Qwen_insert_LoRA, utility_function
import torch
import random
...
batch_sizes = [1, 2, 4, 8, 16, 32, 64, 128, 256, 384, 512, 640, 786, 896, 1024]
epoch = 50
client_number = 30
local_steps = 1
attack_num = 100
...
# 设置模型
num_labels = 5
model_path = "/home/zhike/DGL/Model/Qwen2.5-14B"
...
data_name = '/home/zhike/DGL/Dataset/SST/train.csv'
...
for batch_size in batch_sizes:
    ...
    local_model.layers = local_model.layers[0:max_layer_num]
    Qwen_insert_LoRA.insert_q_proj_lora(local_model, reduction_factor=Adapter_reduction_factor)
    Qwen_insert_LoRA.insert_classification_head(local_model, num_classes=num_labels)
    Qwen_insert_LoRA.freeze_model_parameters(local_model)
    local_model.to(device)

    global_model = copy.deepcopy({k: v.cpu() for k, v in local_model.state_dict().items()})
    ...
    client_datasets = random_split(dataset, client_sizes, generator=generator)  # 3. 随机切分数据
    train_loader_list = [DataLoader(client_dataset, batch_size=batch_size, shuffle=True, generator=generator) for client_dataset in client_datasets]
    ...
    for current_epoch in range(epoch):
        ...
        for current_client in range(client_number):
            ...
            Qwen_insert_LoRA.set_trainable_parameters(local_model)
            ...
            for step in range(local_steps):
                try:
                    ...
                    client_data_iter = iter(client_dataloader)
                    batch = next(client_data_iter)
                input_batch = tokenizer(batch['sentence'], truncation=True, padding=True, return_tensors='pt').to(
                    device)
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            ...
            if current_client == 0:
                for layer in local_model.layers:
                    lora_adapter = layer.self_attn.q_proj.lora
                    weight_grad = lora_adapter.adapter_down.weight.grad
                    weight_divided_bias.append(weight_grad)

            adapter_grads = []
            for block in local_model.layers:
                adapter_grads.append([block.self_attn.q_proj.lora.adapter_down.weight.grad, block.self_attn.q_proj.lora.adapter_down.bias.grad,
                                      block.self_attn.q_proj.lora.adapter_up.weight.grad, block.self_attn.q_proj.lora.adapter_up.bias.grad])
        ...
        # ===============================P-MIA=================================
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True, return_tensors='pt', max_length=512).to(device)
        negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True, return_tensors='pt', max_length=512).to(device)

        local_model.load_state_dict(global_model)
        layer_hook = []
        for layer in local_model.layers:
            temp_hook = layer.self_attn.q_proj.lora.register_forward_pre_hook(capture_adapter_input)
            layer_hook.append(temp_hook)

        adapter_input_captured = []
        local_model.eval()
        with torch.no_grad():
            _ = local_model(**positive_input)
        positive_captured = adapter_input_captured

        adapter_input_captured = []
        with torch.no_grad():
            _ = local_model(**negative_input)
        negative_captured = adapter_input_captured

        positive_temp_res = []
        negative_temp_res = []
        for index, temp_captured in enumerate(positive_captured[1:]):
            _, _, _, res_atol_final = utility_function.can_be_expressed(weight_divided_bias[index+1], temp_captured[0][-1].to(device))
            positive_temp_res.append(res_atol_final.item())
            _, _, _, res_atol_final = utility_function.can_be_expressed(weight_divided_bias[index+1], negative_captured[index+1][0][-1].to(device))
            negative_temp_res.append(res_atol_final.item())
        P_MIA_positive_sample_res.append(abs(min(positive_temp_res)))
        P_MIA_negative_sample_res.append(abs(min(negative_temp_res)))

        del positive_input, negative_input, negative_captured, positive_captured, weight_divided_bias, \
            adapter_input_captured
        torch.cuda.empty_cache()

        for temp in layer_hook:
            temp.remove()

        # ===============================Fed_loss(ICLR, 2023)=================================
        local_model.load_state_dict(fed_loss_local_model)
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        with torch.no_grad():
            out_put = local_model(**positive_input)  # ===========================计算正样本的loss
        ...
        negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        with torch.no_grad():
            out_put = local_model(**negative_input)  # ===========================计算负样本的loss
        ...
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        outputs = local_model(input_ids=positive_input['input_ids'],
        ...
        for block in local_model.layers:
            positive_cosine_grade.append(
                [block.self_attn.q_proj.lora.adapter_down.weight.grad, block.self_attn.q_proj.lora.adapter_down.bias.grad,
                 block.self_attn.q_proj.lora.adapter_up.weight.grad, block.self_attn.q_proj.lora.adapter_up.bias.grad])
        positive_cosine_grade_flat = torch.cat([temp_grade2.flatten() for temp_grade1 in positive_cosine_grade
                                                for temp_grade2 in temp_grade1 if
                                                isinstance(temp_grade2, torch.Tensor)]
                                               , dim=0) if grade else torch.tensor([], device=device)
        positive_cos_sim = torch.cosine_similarity(grade_flat.unsqueeze(0), positive_cosine_grade_flat.unsqueeze(0),
                                                   dim=1)
        ...
        negative_cosine_grade_flat = torch.cat([temp_grade2.flatten() for temp_grade1 in negative_cosine_grade
                                                ...
                                                isinstance(temp_grade2, torch.Tensor)]
                                               , dim=0) if grade else torch.tensor([], device=device)
        negative_cos_sim = torch.cosine_similarity(grade_flat.unsqueeze(0), negative_cosine_grade_flat.unsqueeze(0),
                                                   dim=1)
        ...
        positive_loss_new = loss_fn(positive_output_new,
                                    torch.tensor([positive_sample['labels'].item()]).to(device))
        negative_loss_new = loss_fn(negative_output_new,
                                    torch.tensor([negative_sample['labels'].item()]).to(device))
        ...
        positive_grads = []
        for block in local_model.layers:
            positive_grads.append([block.self_attn.q_proj.lora.adapter_down.weight.grad, block.self_attn.q_proj.lora.adapter_down.bias.grad,
                                   block.self_attn.q_proj.lora.adapter_up.weight.grad, block.self_attn.q_proj.lora.adapter_up.bias.grad])
        ...
        negative_grads = []
        for block in local_model.layers:
            negative_grads.append([block.self_attn.q_proj.lora.adapter_down.weight.grad, block.self_attn.q_proj.lora.adapter_down.bias.grad,
                                   block.self_attn.q_proj.lora.adapter_up.weight.grad, block.self_attn.q_proj.lora.adapter_up.bias.grad])
        ...
        for grade in grade_flat:
            positive_sample_cosine.append(
                torch.cosine_similarity(grade.unsqueeze(0), positive_grads_flat.unsqueeze(0), dim=1).item())
            negative_sample_cosine.append(
                torch.cosine_similarity(grade.unsqueeze(0), negative_grads_flat.unsqueeze(0), dim=1).item())
        ...
        positive_sample_cosine_filter = [value for value in positive_sample_cosine[1:] if
                                         value < positive_Threshold]
        negative_sample_cosine_filter = [value for value in negative_sample_cosine[1:] if
    ...
    # 保存为 JSON 文件
    file_path = f'results/mia_Qwen_LoRA2_results_{batch_size}.json'
    with open(file_path, 'w', encoding='utf-8') as f:
        json.dump(results, f, indent=4, ensure_ascii=False)

    print(f"✅ 当前batch size结果已保存至 '{file_path}'")

    final_result.append([P_MIA_positive_sample_res, P_MIA_negative_sample_res])

file_path = f'results/mia_Qwen_LoRA2_results.json'
with open(file_path, 'w', encoding='utf-8') as f:
    json.dump(final_result, f, indent=4, ensure_ascii=False)

print(f"最终结果已保存至 '{file_path}'")

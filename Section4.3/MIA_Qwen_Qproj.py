...
for batch_size in batch_sizes:
    ...
    for current_epoch in range(epoch):
        ...
        for current_client in range(client_number):
            ...
            Qwen_insert_LoRA.set_trainable_parameters_Qproj(local_model)
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            ...
            if current_client == 0:
                for layer in local_model.layers:
                    ...
                    weight_grad = qproj.weight.grad
                    bias_grad = qproj.bias.grad
                    temp_wei_div_bias = []
            ...
            for block in local_model.layers:
                adapter_grads.append([block.self_attn.q_proj.weight.grad, block.self_attn.q_proj.bias.grad])
        ...
        layer_hook = []
        for layer in local_model.layers:
            temp_hook = layer.self_attn.q_proj.register_forward_pre_hook(capture_adapter_input)
        ...
        local_model.load_state_dict(global_model)  # 复制全局模型到本地，计算样本梯度
        Qwen_insert_LoRA.set_trainable_parameters_Qproj(local_model)
        trainable_params = [p for p in local_model.parameters() if p.requires_grad]
        local_optimizer = torch.optim.Adam(trainable_params, lr=1e-4)
        ...
        for block in local_model.layers:
            positive_cosine_grade.append(
                [block.self_attn.q_proj.weight.grad, block.self_attn.q_proj.bias.grad])
        ...
        positive_cos_sim = torch.cosine_similarity(grade_flat.unsqueeze(0), positive_cosine_grade_flat.unsqueeze(0),
                                                   dim=1)
        Cosine_positive_sample.append(positive_cos_sim.item())  # 保存正样本结果
        ...
        local_model.load_state_dict(fed_loss_local_model)  # 当前batch训练后的模型
        with torch.no_grad():
        ...
        # ===============================FTA(Usenix, 2024)=================================
        mem_start = torch.cuda.memory_allocated(device)  # 统计开始时的显存使用量
        torch.cuda.reset_peak_memory_stats(device)  # 重新开始统计
        time_start = time.time()

        client_weight_updated = updated_models[0]
        local_model.load_state_dict(global_model)  # 复制全局模型到本地
        Qwen_insert_LoRA.set_trainable_parameters_Qproj(local_model)
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True,
                                   return_tensors='pt').to(device)
        negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True,
        ...
        positive_grads = []
        for block in local_model.layers:
            positive_grads.append([block.self_attn.q_proj.weight.grad, block.self_attn.q_proj.bias.grad])
        ...
        negative_grads = []
        for block in local_model.layers:
            negative_grads.append([block.self_attn.q_proj.weight.grad, block.self_attn.q_proj.bias.grad])
    ...
    # 保存为 JSON 文件
    file_path = f'results/mia_Qwen_Qproj_results_{batch_size}.json'
...
file_path = f'results/mia_Qwen_Qproj_results.json'
with open(file_path, 'w', encoding='utf-8') as f:

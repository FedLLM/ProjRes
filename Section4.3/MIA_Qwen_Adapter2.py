from auxiliary_function import Qwen_insert_Adapter, utility_function
import torch
import random
...
for batch_size in batch_sizes:
    ...
    Qwen_insert_Adapter.insert_transformer_adapters(local_model, reduction_factor=Adapter_reduction_factor, layer_start=0, layer_end=len(local_model.layers))  # 插入adapter
    Qwen_insert_Adapter.insert_classification_head(local_model, num_classes=num_labels)  # 插入分类头
    Qwen_insert_Adapter.freeze_model_parameters(local_model)  # 冻结模型预训练参数
    ...
    for current_epoch in range(epoch):
        ...
        for current_client in range(client_number):
            ...
            Qwen_insert_Adapter.set_trainable_parameters(local_model)
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            client_dataloader = train_loader_list[current_client]  # 抽取一个batch，开始执行训练并攻击
            ...
            local_model.load_state_dict(global_model)
            Qwen_insert_Adapter.set_trainable_parameters(local_model)
            local_optimizer = torch.optim.Adam(local_model.parameters(), lr=1e-4)  # 假设的学习率
            ...
            if current_client == 0:
                for layer in local_model.layers:
                    ...
                    bias_grad = adapter_down.bias.grad  # 偏置梯度
                    temp_wei_div_bias = []  # 用于保存权重偏执梯度比
            ...
            local_optimizer.step()  # 更新本地模型

            if current_client == 0:
                fed_loss_local_model = copy.deepcopy({k: v.cpu() for k, v in local_model.state_dict().items()})
            trainable_params = [p.detach().cpu().clone() for p in local_model.parameters() if p.requires_grad]
        ...
        layer_hook = []
        for layer in local_model.layers:
            temp_hook = layer.adapter.register_forward_pre_hook(capture_adapter_input)
        ...
        positive_input = tokenizer(positive_sample['sentence'], truncation=True, padding=True, return_tensors='pt').to(
            device)
        with torch.no_grad():
        ...
        negative_input = tokenizer(negative_sample['sentence'], truncation=True, padding=True, return_tensors='pt').to(
            device)
        with torch.no_grad():
    ...
    # 保存为 JSON 文件
    file_path = f'results/mia_Qwen_Adapter1.2_results_{batch_size}.json'
...
file_path = f'results/mia_Qwen_Adapter1.2_results.json'
with open(file_path, 'w', encoding='utf-8') as f:

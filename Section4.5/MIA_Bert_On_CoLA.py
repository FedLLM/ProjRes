...
Bert_insert_Adapter.insert_transformer_adapters(global_model, reduction_factor=Adapter_reduction_factor, layer_start=layer_index, layer_end=layer_index+1)
...
Bert_insert_Adapter.insert_transformer_adapters(local_model, reduction_factor=Adapter_reduction_factor, layer_start=layer_index, layer_end=layer_index+1)
...
for batch_size in batch_sizes:
    ...
    train_loader_list = [  # 4. 为每个客户端创建 DataLoader
    ...
    for current_attack_number in range(attack_num):
        ...
        for current_client in range(client_number):
            ...
            loss.backward()
            adapter_grads = []
            block = local_model.encoder.layer[layer_index]
        ...
        grade = grades[0]
        weight_divided_bias = []
        for grade_temp in grade:
            weight_grad = grade_temp[0]
            bias_grad = grade_temp[1]
            temp_wei_div_bias = []
        ...
        hook = local_model.encoder.layer[layer_index].adapter.register_forward_pre_hook(get_hook(0))
        hooks.append(hook)
        ...
        for index, temp_captured in enumerate(adapter_inputs):
            is_close, _, _, res_atol_final = utility_function.can_be_expressed(weight_divided_bias[index], temp_captured[1][0][0].to(device))
    ...
    # 保存为 JSON 文件
    file_path = f'results\mia_bert_cola_results_{batch_size}_layer_index_{layer_index}.json'

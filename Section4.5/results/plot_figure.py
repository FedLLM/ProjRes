...
batch_size = 16
base_dir = "."
target_layers = np.arange(12)   # 0 到 11
file_template = "mia_bert_cola_results_{}_layer_index_{{}}.json".format(batch_size)

output_image_name = f"auc_vs_layer_batch{batch_size}.pdf"
...
for layer_idx in target_layers:
    ...
    for method in methods_config:
        ...
        try:
            ...
            print(f"⚠️  {name} 在 layer {layer_idx} 计算 AUC 失败: {e}")
            method_auc_data[name].append(np.nan)

# ------------------- 绘图：AUC vs Layer Index -------------------
...
plt.xlabel('Layer Index (Adapter Insertion Position)')
plt.ylabel('AUC')
plt.ylim(0.15, 1.02)
